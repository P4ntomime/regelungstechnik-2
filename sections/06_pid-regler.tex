...
\begin{minipage}[t]{0.48\columnwidth}
    \begin{center}
        \myul{PI-Regler ($K_R$ und $T_N$ einstellbar)}
    \end{center}
    $$ u(t) = \frac{1}{T_N} \cdot \Big( \int\limits_0^t e(\tau) \, \diff \tau + e(t) \Big) K_R  \cdot K_R $$

    % TODO: Frequenzbereich -> UTF
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\columnwidth}
    \begin{center}
...
% offenen Regelkreis anschauen
% Parameter von Regler so wählen, dass man Pol und NS kürzen kann
% ideal: Regelstrecke ist nur noch ein Integrator, das non-plus-ultra der Regelungstechnik
