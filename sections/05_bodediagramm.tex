...
\begin{itemize}
    ...
    \item \textbf{Instabilität}:  $K_{RES} < 0$ \textbf{und} $\Phi_{RES} < 0$ (ergibt sich automatisch, wenn einer der 
        beiden Parameter $< 0$ ist)
    \item \textbf{Stabilität}: $K_{RES} > 0$ \textbf{und} $\Phi_{RES} > 0$ 
...
\subsection{Alternative Stabilitätskriterien -- Vorzeichenregel}
