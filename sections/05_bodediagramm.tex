...
\begin{align*}
    ...
    \frac{Y(s)}{U(s)}                       &= \frac{\text{Zähler}}{\cbl{a_n s^n + a_{n-1} s^{n-1} + \ldots + a_2 s^2  + a_1 s + a_0 } } \\
    \frac{Y(\jimg \omega)}{U(\jimg \omega)} &= \frac{\text{Zähler}}{\cbl{a_n (\jimg \omega)^n + a_{n-1} (\jimg \omega)^{n-1} + \ldots + a_2 (\jimg \omega)^2  + a_1 (\jimg \omega) + a_0 } }
\end{align*}
